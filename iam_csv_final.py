...
def read_user_data_from_csv(csv_file):
    ...
    try:
        ...
        for _, row in df.iterrows():
            user_name = row['email']  # Adjust the key to 'name' to match the actual column header
...
def generate_random_password():
    return "Learner@HV2025"  # Example password with lowercase, uppercase, digits, and special characters
...
def main():
    herovired_group = create_iam_group("heroviredB14")

    # Replace 'users.csv' with the actual name of your CSV file.
    user_data = read_user_data_from_csv('users.csv')

    table = PrettyTable()
    table.field_names = ["User", "Group", "Console Sign-in URL", "Username", "Temporary Password"]
    ...
    for data in user_data:
        ...
        if user_created:
            ...
            add_permissions(user, [], 'us-west-1')  # Empty services list as no additional permissions needed
            ...
            table.add_row([user, group, console_signin_url, user, temporary_password])

    # Save PrettyTable to CSV file
